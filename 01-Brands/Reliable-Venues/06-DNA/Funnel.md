---
brand: Reliable-Venues
area: dna
domain: funnel
note_type: canonical
status: active
canonical: true
used_for_ai: true
owner: Luis Torres
last_updated: 2026-05-21
sensitivity: internal
hub_role: child
up:
  - "[[01-Brands/Reliable-Venues/06-DNA/DNA-Home]]"
related:
  - "[[01-Brands/Reliable-Venues/06-DNA/Conversion]]"
  - "[[01-Brands/Reliable-Venues/06-DNA/Sales]]"
  - "[[01-Brands/Reliable-Venues/06-DNA/Money-Model]]"
---

# RV Funnel — DNA

## Parent
- [[01-Brands/Reliable-Venues/06-DNA/DNA-Home|RV DNA Home]]

## Related
- [[01-Brands/Reliable-Venues/06-DNA/Conversion|RV Conversion]]
- [[01-Brands/Reliable-Venues/06-DNA/Sales|RV Sales]]
- [[01-Brands/Reliable-Venues/06-DNA/Money-Model|RV Money Model]]

## Funnel DNA

Website → strategy call form → calendar booking → live call → fit decision → implementation path.

## Primary CTA
Book a Strategy Call (Venue Scaling Roadmap)

## Secondary CTA
See How It Works (walkthrough video)
