---
brand: Orlando-Event-Venue
area: systems
subarea: marketing
note_type: home
status: active
canonical: true
used_for_ai: true
source_type: curated
source_reference: "OEV-PROJECT docs/marketing + docs/superpowers specs + ClickUp OEV/GOOGLE ADS"
owner: Cristian
last_updated: 2026-08-20
sensitivity: internal
hub_role: system-hub
tags:
  - oev
  - marketing
up:
  - "[[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home]]"
down:
  - "[[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Google-Ads-Post-Mortem-2026-06]]"
  - "[[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Lead-Magnet-Event-Planning-Kit]]"
  - "[[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Website-One-Page-Redesign-2026-07]]"
related:
  - "[[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Lead-Definition]]"
  - "[[01-Brands/Orlando-Event-Venue/06-DNA/Lead]]"
  - "[[01-Brands/Orlando-Event-Venue/06-DNA/Conversion]]"
  - "[[01-Brands/Orlando-Event-Venue/02-Communication/Templates/Pop-Up-Lead-Magnet-Sequence]]"
---

# OEV Marketing Home

## Parent
- [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|OEV Brand Home]]

## Children
- [[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Google-Ads-Post-Mortem-2026-06|Google Ads Post-Mortem — June 2026]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Lead-Magnet-Event-Planning-Kit|Lead Magnet — Event Planning Kit]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Website-One-Page-Redesign-2026-07|Website One-Page Redesign — July 2026]]

## Related
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Lead-Definition|OEV Lead Definition]]
- [[01-Brands/Orlando-Event-Venue/06-DNA/Lead|OEV Lead DNA]]
- [[01-Brands/Orlando-Event-Venue/06-DNA/Conversion|OEV Conversion DNA]]
- [[01-Brands/Orlando-Event-Venue/02-Communication/Templates/Pop-Up-Lead-Magnet-Sequence|Pop-Up Lead Magnet Sequence]]

## What this hub covers
Paid acquisition, the website as a conversion surface, and the lead magnet. This hub was previously listed as a deferred system in [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|OEV Brand Home]]; it is now active because real campaign and website work happened between June and August 2026.

## Current state (2026-08-20)
- **Paid:** Google Ads Performance Max ran in June 2026 and failed for measurable, documented reasons. No new spend until the blockers in the post-mortem are cleared.
- **Site:** rebuilt as a one-page in July 2026, styled on the DR structure with OEV branding, with legacy routes redirected to anchors.
- **Lead magnet:** Event Planning Kit with the PLAN50 offer, replacing HOST100 / SAVE100 / SAVE50, live since 2026-08-04.
- **Instrumentation:** GA4 purchase conversion now fires on booking confirmation. Before July 2026, no conversion event existed at all.

## Standing rule
Never buy traffic into a funnel whose capture path has not been tested in production that week. The June campaign paid for visitors while the popup silently rejected every submission for six weeks.
