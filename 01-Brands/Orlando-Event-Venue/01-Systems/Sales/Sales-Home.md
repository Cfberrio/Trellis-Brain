---
brand: Orlando-Event-Venue
area: systems
subarea: sales
note_type: home
status: active
canonical: true
used_for_ai: true
source_type: derived
source_reference: "OEV lead + conversion DNA"
owner: Luis
last_updated: 2026-04-21
sensitivity: internal
related_systems: [marketing, finance]
related_notes: ["[[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]", "[[01-Brands/Orlando-Event-Venue/01-Systems/Finance/Pricing-Logic|OEV Pricing Logic]]"]
hub_role: system-home
tags: [oev, sales-home]
---

# OEV Sales Home

## Parent
- [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]

## Children
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Lead-Definition|OEV Lead Definition]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Sales-Process|OEV Sales Process]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Follow-Up-Rules|OEV Follow-Up Rules]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Objection-Handling|OEV Objection Handling]]
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Closing-Logic|OEV Closing Logic]]

## Related
- [[01-Brands/Orlando-Event-Venue/01-Systems/Finance/Pricing-Logic|OEV Pricing Logic]]

## Role
The OEV sales system turns date-driven inquiry into deposit, then into confirmed event, then into add-ons, proof, and repeat business.
