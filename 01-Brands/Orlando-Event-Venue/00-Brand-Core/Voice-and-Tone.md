---
brand: Orlando-Event-Venue
area: brand-core
subarea: voice-tone
note_type: core
status: active
canonical: true
used_for_ai: true
source_type: curated
source_reference: "Derived from brand logic and offer structure"
owner: Luis
last_updated: 2026-04-21
sensitivity: internal
related_systems: [marketing, sales]
related_notes: ["[[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]", "[[01-Brands/Orlando-Event-Venue/01-Systems/Marketing/Marketing-Home|OEV Marketing Home]]", "[[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Sales-Home|OEV Sales Home]]"]
hub_role: child
tags: [oev, voice, tone]
---

# OEV Voice and Tone

## Parent
- [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]

## Related
- [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Language-Rules|OEV Language Rules]]

## Voice Definition
OEV should sound:
- clear,
- composed,
- practical,
- trustworthy,
- logistics-aware,
- professionally warm without sounding fluffy.

## Tone by Situation
### Website / landing pages
Confident, simple, fast to understand, credibility-first.

### Pricing and rules
Direct, calm, transparent, non-defensive.

### Booking and follow-up
Helpful, decisive, next-step oriented.

### Objection handling
Empathetic but structured. Do not ramble or over-explain.

## What To Avoid
- Overly luxurious fantasy language.
- Generic banquet-hall language.
- Overly casual slang.
- Apologetic language when stating rules.
- Hypey direct-response language that reduces trust.
