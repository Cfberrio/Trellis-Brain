---
brand: Orlando-Event-Venue
area: brand-core
subarea: objections
note_type: core
status: active
canonical: true
used_for_ai: true
source_type: derived
source_reference: "OEV DNA PDF"
owner: Luis
last_updated: 2026-04-21
sensitivity: internal
related_systems: [sales]
related_notes: ["[[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]", "[[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Sales-Home|OEV Sales Home]]"]
hub_role: child
tags: [oev, objections]
---

# OEV Objections

## Parent
- [[01-Brands/Orlando-Event-Venue/00-Brand-Core/Brand-Home|Orlando Event Venue - Brand Home]]

## Related
- [[01-Brands/Orlando-Event-Venue/01-Systems/Sales/Objection-Handling|OEV Objection Handling]]

## Primary Objections
1. The total feels expensive after cleaning and production are added.
2. The rules feel strict and carry penalty risk.
3. A/V sounds complicated or expensive.
4. The client asks about alcohol or bar service → this is not an objection; it is a bar service upsell opportunity. Response: "Bar service is available as an add-on — we handle the coordination. Packages start at $18/guest. Want me to include that in your quote?"
5. Setup and reset feel like too much work.

## Handling Principle
Do not fight objections with hype. Resolve them by clarifying fit, economics, and tradeoffs. If the lead is not a fit, exit cleanly instead of twisting the offer.
