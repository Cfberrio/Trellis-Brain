---
brand: Discipline-Rift
domain: brand-core
area: brand-core
note_type: home
status: active
canonical: true
used_for_ai: true
source_type: curated
owner: Luis
last_updated: 2026-04-23
sensitivity: internal
hub_role: brand-hub
---

# Discipline Rift — Brand Home

## Parent
- [[00-Trellis-Core/Trellis-Home|Trellis Home]]

## Children
- [[01-Brands/Discipline-Rift/00-Brand-Core/Positioning|DR Positioning]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Avatar|DR Avatar]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Offers|DR Offers]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Value-Proposition|DR Value Proposition]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Voice-and-Tone|DR Voice and Tone]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Objections|DR Objections]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Constraints|DR Constraints]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Opportunities|DR Opportunities]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/KPIs|DR KPIs]]
- [[01-Brands/Discipline-Rift/01-Systems/Systems-Home|DR Systems Home]]
- [[01-Brands/Discipline-Rift/01-Systems/Sales/Sales-Home|DR Sales Home]]
- [[01-Brands/Discipline-Rift/06-DNA/DNA-Home|DR DNA Home]]
- [[01-Brands/Discipline-Rift/05-Operations/Training/Training-Home|DR Training Home]]
- [[01-Brands/Discipline-Rift/02-Communication/Communication-Home|DR Communication Home]]
- [[01-Brands/Discipline-Rift/04-Projects/DRF/DRF-Home|DRF Home (franchise/licensee)]]

## Related
- [[01-Brands/Discipline-Rift/06-DNA/Market|DR Market]]
- [[01-Brands/Discipline-Rift/06-DNA/Problem|DR Problem]]
- [[01-Brands/Discipline-Rift/06-DNA/Offer|DR Offer]]
- [[01-Brands/Discipline-Rift/06-DNA/Message|DR Message]]
- [[01-Brands/Discipline-Rift/06-DNA/Funnel|DR Funnel]]
- [[01-Brands/Discipline-Rift/06-DNA/Lead|DR Lead]]
- [[01-Brands/Discipline-Rift/06-DNA/Conversion|DR Conversion]]
- [[01-Brands/Discipline-Rift/06-DNA/Money-Model|DR Money Model]]
- [[01-Brands/Discipline-Rift/06-DNA/Metrics|DR Metrics]]

## Deferred Systems
These systems are planned for DR but notes do not yet exist. Do not link from active routing until built.
- Marketing-Home — DR marketing system hub
- Onboarding-Process — parent and student onboarding
- Delivery-Process — session delivery
- Retention-Strategy — retention and re-enrollment
- Pricing-Logic — season pricing and payment rules

## One-line models
**For parents:** Discipline Rift helps parents of kids 6–12 get their child active, confident, and connected through fun-first, on-campus after-school sports seasons led by trained coaches and designed for beginner-friendly development.

**For schools:** Discipline Rift helps schools increase youth sports participation with low-lift, on-campus after-school seasons that are parent-funded and operationally clean.

**For the business:** Revenue comes from parent-paid, per-player, fixed-term seasons delivered on campus with the school providing access, space, and distribution support.

## Canon decisions currently locked
- Canon price per season: **$129**.
- Volleyball is the current spearhead sport because it has the strongest demand, most teams, and most students.
- Tennis, pickleball, and flag football also fill, but volleyball remains the default proof-of-demand sport.
- Communication, GHL, and tools are intentionally excluded from this version.

## What Discipline Rift is really selling
Discipline Rift is not selling “sports practice” in the abstract. It is selling the easiest credible yes for youth sports participation: on-campus, after-school, beginner-friendly sports seasons that reduce parent logistics, lower intimidation, and create visible progress without the intensity and friction of club sports.

## Strategic center of gravity
The model wins when four things stay true at the same time:
1. The school experiences low operational burden.
2. The parent feels the offer is simple, safe, and worth the fixed seasonal commitment.
3. The child feels the program is beginner-safe and fun enough to return.
4. Delivery produces enough progress, belonging, and consistency that re-enrollment becomes natural.

## Operational implications
- The model is logistics-driven as much as it is sports-driven.
- The dismissal/pickup flow is part of the offer, not just an operational detail.
- Coach consistency and practice quality are part of the sales engine because they increase trust and retention.
- Attendance is not just a delivery KPI; it is a leading indicator of retention and future season conversion.

## Sports knowledge system
DR now contains a structured training library under [[Training-Home]]. That library is divided into:
- shared coaching resources,
- by-sport folders,
- imported PDFs and books,
- one hub note per sport so the graph stays connected.

## Open decisions not yet canonized
- Whether DR will use a public guarantee and, if so, what kind.
- Whether continuity offers like Home Club will be active canon or remain backlog concepts.
- Whether pickleball needs its own formal curriculum import beyond the current structural placeholder.


## Current Sport Reality
- Volleyball is currently the strongest spearhead in volume, team count, and student demand.
- Tennis, pickleball, and flag football also fill and matter strategically, but volleyball remains the clearest lead sport for positioning and proof.
