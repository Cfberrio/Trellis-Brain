---
brand: Discipline-Rift
area: dna
note_type: home
status: active
canonical: true
used_for_ai: true
source_type: curated
sensitivity: internal
hub_role: dna-hub
last_updated: 2026-08-09
---

# DNA Home

## Parent
- [[01-Brands/Discipline-Rift/00-Brand-Core/Brand-Home|DR Brand Home]]

## Children
- [[01-Brands/Discipline-Rift/06-DNA/Market|DR Market]]
- [[01-Brands/Discipline-Rift/06-DNA/Problem|DR Problem]]
- [[01-Brands/Discipline-Rift/06-DNA/Problem-Map|DR Problem Map]] — the 29-problem expansion of Problem, across parents, coaches, students, and system
- [[01-Brands/Discipline-Rift/06-DNA/Offer|DR Offer]]
- [[01-Brands/Discipline-Rift/06-DNA/Message|DR Message]]
- [[01-Brands/Discipline-Rift/06-DNA/Funnel|DR Funnel]]
- [[01-Brands/Discipline-Rift/06-DNA/Lead|DR Lead]]
- [[01-Brands/Discipline-Rift/06-DNA/Conversion|DR Conversion]]
- [[01-Brands/Discipline-Rift/06-DNA/Money-Model|DR Money Model]]
- [[01-Brands/Discipline-Rift/06-DNA/Delivery|DR Delivery]]
- [[01-Brands/Discipline-Rift/06-DNA/Metrics|DR Metrics]]

## Related
- [[01-Brands/Discipline-Rift/00-Brand-Core/Positioning|DR Positioning]]
- [[01-Brands/Discipline-Rift/00-Brand-Core/Value-Proposition|DR Value Proposition]]
- [[01-Brands/Discipline-Rift/05-Operations/Training/Training-Home|DR Training Home]]
- [[01-Brands/Discipline-Rift/01-Systems/Sales/Sales-Home|DR Sales Home]]

## Purpose
The strategic skeleton of DR: where it plays, what pain it solves, what it sells, how it makes the offer known, how it converts, how it captures money, how it delivers, and what it measures.

## How To Read This Folder

1. **Market** — where DR plays (submarket, winnable wedge, competitor set, "we win here because...")
2. **Problem** — what pain we solve
3. **Offer** — what we sell + why it's compelling
4. **Message** — the core message
5. **Funnel** — how leads flow
6. **Lead** — how we make ourselves known + what to count
7. **Conversion** — how we close (nurture + close + objection handling)
8. **Money Model** — customer journey ladder (entry / core / upsells / continuity / downsells)
9. **Delivery** — how results are produced
10. **Metrics** — what we measure

> The short notes here are strategic skeleton. Operational scripts and SOPs live under [[01-Brands/Discipline-Rift/01-Systems/Sales/Sales-Home|Sales Home]] and [[01-Brands/Discipline-Rift/05-Operations/SOPs/SOPs-Home|Operations / SOPs Home]].
