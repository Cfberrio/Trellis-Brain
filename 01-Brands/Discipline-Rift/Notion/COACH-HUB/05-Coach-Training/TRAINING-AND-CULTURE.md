---
brand: Discipline-Rift
area: notion-mirror
note_type: notion-page
source_type: notion_mirror
notion_page_id: 6ab04528-85a8-8370-b557-013b3453a92c
notion_url: https://app.notion.com/p/TRAINING-CULTURE-6ab0452885a88370b557013b3453a92c
notion_db: COACH HUB
section: Coach Training
page_type: Hub
sport: All Sports
status: Live
order: 6
notion_created: 2026-08-11
notion_last_edited: 2026-09-17
mirrored: 2026-09-17
mirror_complete: true
aliases:
  - "TRAINING & CULTURE"
  - "TRAINING AND CULTURE"
  - "COACH TRAINING"
---
<callout icon="📢" color="gray_bg">
	Coach! This is where you get better. The growth of your players mirrors yours. Read this when you are not about to coach. Every **→** below is a door: tap it.
</callout>
<callout icon="💡" color="gray_bg">
	The pages under **WHO WE ARE**, **WHO YOU ARE AS A COACH** and **HOW YOU GET BETTER** are the standing rules: they stay true every season. The pages under **CONFERENCES** are the record of what we taught on a given day, and who taught it.
</callout>
# TRAINING & CULTURE
<callout icon="⛰️" color="gray_bg">
	**WHO WE ARE**
	[[MISSION & VISION]]
	*Why DR exists and where it is going.*
	[[01-Brands/Discipline-Rift/Notion/COACH-HUB/05-Coach-Training/CORE-VALUES|CORE VALUES]]
	*Humility, perseverance, adaptability.*
	[[OUR CULTURE]]
	*How we behave around kids, parents and each other.*
</callout>
<callout icon="/icons/brain_green.svg" color="gray_bg">
	**WHO YOU ARE AS A COACH**
	[[01-Brands/Discipline-Rift/Notion/COACH-HUB/05-Coach-Training/BEING-A-COACH|BEING A COACH]]
	*What the role means at DR.*
	[[01-Brands/Discipline-Rift/Notion/COACH-HUB/05-Coach-Training/THE-HAND-OF-A-COACH|THE HAND OF A COACH]]
	*The five fingers of coaching, created by Coach Luis Torres.*
	[[FUELED]]
	*The behavioural standard.*
	[[01-Brands/Discipline-Rift/Notion/COACH-HUB/05-Coach-Training/DR-FORMULA|DR FORMULA]]
	*The coaching formula.*
	[[COACH EXPECTATIONS]]
	*The ten responsibilities. The practical version lives in START HERE as Coach Non-Negotiables.*
</callout>
<callout icon="/icons/gradebook_green.svg" color="gray_bg">
	**HOW YOU GET BETTER**
	[[1% SOLUTIONS]]
	*One hundred small improvements.*
	[[COACHING TERMS]]
	*The one glossary, every DR term.*
	[[READING SHELF]]
	*The DR reading list. Seven books, one line each on why it matters on our court.*
</callout>
<callout icon="🏛️" color="gray_bg">
	**CONFERENCES**
	[[2023 CONFERENCE]]
	*What we taught in 2023.*
	[[2024 CONFERENCE]]
	*What we taught in 2024.*
	[[2025 CONFERENCE]]
	*What we taught in 2025.*
	[[WORKSHOPS]]
	*Placeholder: no source material exists yet.*
</callout>
## TAKE ACTION
<callout icon="🚩" color="gray_bg">
	Start with WHO WE ARE and WHO YOU ARE AS A COACH. Those are the standing rules you coach by every practice. Read the conference pages second: they are the record of past trainings, how we got to these rules.
</callout>
<callout icon="🚩" color="gray_bg">
	Each week: pick one page from this hub and try one thing from it at your next practice that you have not done before. You cannot take players somewhere you have not gone: be students of leadership.
</callout>
## SUB-PAGES (nested under this hub in Notion)
- TERMINOLOGY (`2ed0452885a881a78ff7d0e68138c2dc`) — legacy DRVC glossary page nested here; superseded by [[COACHING TERMS]]. Not mirrored separately.
