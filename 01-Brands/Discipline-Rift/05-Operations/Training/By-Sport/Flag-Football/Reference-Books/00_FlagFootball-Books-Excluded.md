...
Fuente: "The Field Library" — investigación de currículo de flag football elemental (K-5), Discipline Rift, sept. 2026.

Nota de método: el informe original tenía 51 recursos numerados. Solo 15 traían URL directa en el HTML entregado (recursos 1-15, tiers 1-2). Para los otros 36 (tiers 3-6) se hizo búsqueda web activa para encontrar la URL real de cada uno y verificarla contra el título/autor/editor descrito — no se inventó ninguna URL. El único recurso sin nota individual en esta carpeta es el #04 (enlace muerto confirmado, sin snapshot en Wayback). Todos los demás — incluidos los gateados, de pago o parcialmente accesibles — sí tienen nota individual con los metadatos reales confirmados; esta página los cataloga con el motivo real y enlaza a su nota.

> [!warning] Extracción estructurada, no copia verbatim
> El plan original pedía texto completo verbatim de cada PDF. Un subagente marcó correctamente que reproducir copias completas de obras con copyright de terceros — aunque sea para uso interno en un vault privado — no es algo que se deba hacer solo porque no se redistribuye. Se cambió el método para las 51 fuentes: cada nota tiene metadatos bibliográficos completos, el desglose de cada actividad/lección nombrada con su propósito, todas las reglas y números clave citados directamente, y citas cortas solo para líneas que cargan el sentido (una definición, una instrucción clave). Es exhaustivo en estructura y hechos, no una copia palabra por palabra del documento fuente.

## Sin copia recuperable, sin nota individual

- **#04 — Flag Football (Intermediate, Grades 3-5), OPEN / US Games** — `openphysed.org/wp-content/uploads/2024/10/I-24-000-FlagFootball-CompletePacket.pdf` — HTTP 404 confirmado en la URL original; redirige a `open.varsityuniversity.org`, que también 404 (dominio raíz devuelve 403). Wayback CDX API + availability API devuelven cero snapshots para esta URL exacta. Mismo patrón de migración de dominio roto que ya se documentó para OPEN en la biblioteca de pickleball — pero ahí sí hubo snapshots recuperables; aquí no los hay. **Si este documento vuelve a estar disponible, es el módulo de flag football mejor construido del informe original (67 pp., 8 actividades) — vale la pena reintentar más adelante.**
