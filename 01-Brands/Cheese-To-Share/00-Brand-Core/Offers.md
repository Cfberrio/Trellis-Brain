---
brand: Cheese-To-Share
area: brand-core
note_type: canonical
status: active
used_for_ai: true
owner: Luis
last_updated: 2026-04-23
sensitivity: internal
hub_role: child
up:
  - "[[01-Brands/Cheese-To-Share/00-Brand-Core/Brand-Home]]"
down:
  - "[[01-Brands/Cheese-To-Share/00-Brand-Core/Value-Proposition]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Finance/Pricing-Logic]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Delivery/Delivery-Process]]"
related:
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Menu]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Catering]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Location]]"
---

# Offers

## Parent
- [[01-Brands/Cheese-To-Share/00-Brand-Core/Brand-Home|CTS Brand Home]]

## Children
- [[01-Brands/Cheese-To-Share/00-Brand-Core/Value-Proposition|CTS Value Proposition]]
- [[01-Brands/Cheese-To-Share/01-Systems/Finance/Pricing-Logic|CTS Pricing Logic]]
- [[01-Brands/Cheese-To-Share/01-Systems/Delivery/Delivery-Process|CTS Delivery Process]]

## Related
- [[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Menu|CTS Menu Page]]
- [[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Catering|CTS Catering Page]]
- [[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Location|CTS Location Page]]

## Core offer stack

### Everyday / menu offers
- Cheese Boards
- Flatbreads
- To Share items
- Sandwiches
- Brunch Toasts
- Cheese Bowls
- Desserts

### Catering offers
- Cheese Boards (catering sizes)
- Grazing Table Catering
- Island Package
- Floral Rustic Package
- Experience Stations
- Custom Styled Package (lead capture only for now, not direct paid conversion)

## CTA logic
- **Order Now** = daily order intent → Location / ordering options
- **Book Now** = catering intent → inquiry / booking flow

## Offer architecture rule
CTS must keep **daily ordering** and **catering booking** separate. The menu page sells immediate food intent. The catering page sells event intent.

## High-value catering anchors
- Cheese Boards (easy packaged entry offer)
- Grazing Table (visual social experience)
- Floral Rustic Package (higher perceived value)
- Experience Stations (premium add-on lever)

## Operational note
Custom Styled Package should currently function as a **lead-generation / consultation path**, not a direct checkout path, because the service is not standardized enough for instant payment.
