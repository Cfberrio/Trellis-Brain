---
brand: Cheese-To-Share
area: systems
domain: website-assets
note_type: canonical
status: active
used_for_ai: true
---

# Assets

## Parent
- [[Sitemap]]

## Related
- [[CTS-Material-Final-Product-Library]]
- [[Menu]]
- [[Catering]]
- [[Content-Strategy]]

## Asset system purpose
This note tracks the usable website-facing asset pool for CTS:
- product docs
- product naming
- product ingredient references
- short-form video references
- visual direction references

## Available structured source material
The uploaded “Material Final” zip contains step-by-step English and Spanish docs for menu items across:
- Cheese Boards
- Flatbreads
- Shareables / To Share
- Sandwiches
- Toasts
- Bread Bowls / Cheese Bowls
- Desserts

There is also at least one product video asset currently visible in the material set.

## Website asset priorities
1. Menu item photos / hero visuals
2. Catering visuals
3. Product-level reference docs
4. Short-form motion assets
5. Story / founder visuals

## Practical asset rule
For the website, visual clarity matters more than showing every single production detail. The long-form product docs should support copy and consistency, not be pasted directly into the website.

## Fast asset inventory
- Structured source library: [[CTS-Material-Final-Product-Library]]
- Design style reference: Emergence
- Menu layout reference: La Piscina del Pedroso
