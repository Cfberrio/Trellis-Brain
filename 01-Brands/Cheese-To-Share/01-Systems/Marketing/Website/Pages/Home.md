---
brand: Cheese-To-Share
area: systems
domain: website-page
note_type: canonical
status: active
used_for_ai: true
up:
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Sitemap]]"
related:
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Menu]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Catering]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Location]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Story]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Pages/Contact]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Marketing/Website/Copy]]"
---

# Home

## Parent
- [[Sitemap]]

## Related
- [[Menu]]
- [[Catering]]
- [[Location]]
- [[Story]]
- [[Contact]]
- [[Copy]]

## Hero structure
- Headline: Cheese To Share
- Subheadline: European foods for moments that matter
- Supporting line: We love to share
- Buttons: Explore Menu, Request Catering, Visit Us

## Experience direction
- Immersive
- Cinematic
- Dynamic scroll
- Elegant transitions

## Job of the page
- establish brand
- establish feeling
- immediately route user by intent:
  - menu intent
  - catering intent
  - visit / order intent

## Scroll sequence
Hero → Menu preview → Catering preview → Location → Story → Contact/Footer
