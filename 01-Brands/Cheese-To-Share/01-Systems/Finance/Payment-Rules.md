---
brand: Cheese-To-Share
area: systems
domain: finance
note_type: canonical
status: active
used_for_ai: true
up:
  - "[[01-Brands/Cheese-To-Share/01-Systems/Finance/Pricing-Logic]]"
related:
  - "[[01-Brands/Cheese-To-Share/01-Systems/Finance/Collection-Process]]"
  - "[[01-Brands/Cheese-To-Share/01-Systems/Finance/Refund-Rules]]"
---

# Payment Rules

## Parent
- [[01-Brands/Cheese-To-Share/01-Systems/Finance/Pricing-Logic|CTS Pricing Logic]]

## Related
- [[01-Brands/Cheese-To-Share/01-Systems/Finance/Collection-Process|CTS Collection Process]]
- [[01-Brands/Cheese-To-Share/01-Systems/Finance/Refund-Rules|CTS Refund Rules]]

## Confirmed payment rules
- 50% deposit required
- remaining balance due 24 hours before the event
- final quotes by Luis Torres

## Operational meaning
No event should be treated as fully locked without the deposit.
